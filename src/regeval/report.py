...
def render_diff_markdown(diff: DiffResult) -> str:
    """Render a DiffResult as a GitHub-Flavored Markdown table, suitable
    for posting as a pull request comment (via the regeval GitHub Action).
    """
    lines = ["## regeval regression report", ""]

    if diff.has_regressions:
        lines.append(f"⚠️ **{len(diff.regressed)} regression(s) detected**")
    else:
        lines.append("✅ **No regressions detected**")
    if diff.improved:
        lines.append(f"📈 {len(diff.improved)} case(s) improved")
    lines.append("")

    lines.append("| Case | Status | Baseline | Current | Δ |")
    lines.append("|---|---|---|---|---|")
    ordered = diff.regressed + diff.improved + diff.new + diff.removed + diff.unchanged
    status_emoji = {
        DiffStatus.REGRESSED: "🔴 REGRESSED",
        ...
        DiffStatus.UNCHANGED: "⚪ unchanged",
        DiffStatus.NEW: "🆕 NEW",
        DiffStatus.REMOVED: "🗑️ REMOVED",
    }

    for case_diff in ordered:
        label = status_emoji[case_diff.status]
        baseline_str = f"{case_diff.baseline_score:.2f}" if case_diff.baseline_score is not None else "—" #noqa
        current_str = f"{case_diff.current_score:.2f}" if case_diff.current_score is not None else "—" #noqa
        delta_str = f"{case_diff.score_delta:+.2f}" if case_diff.score_delta is not None else "—"
        lines.append(f"| {case_diff.id} | {label} | {baseline_str} | {current_str} | {delta_str} |")

    lines.append("")
    lines.append("*Generated by [regeval](https://github.com/raoamogh/regeval)*")
